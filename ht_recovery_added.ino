...
void CalcError() {
  ...
  if (lineDetected) {
    lostLineTimer = 0;  // reset timer when line is found
    ...
    if (lostLineTimer == 0) {
      lostLineTimer = millis();  // start timer when line first lost
    }

    if (millis() - lostLineTimer > 500) {
      // lost for too long, reverse briefly
      runMotorAtSpeed(LEFT, -60);
      runMotorAtSpeed(RIGHT, -60);
      delay(300);
      lostLineTimer = 0;
    } else {
      // spin toward last known direction
      error = (lasterror > 0) ? 3.5 : -3.5;
    }
  }
} // end CalcError()
// ************************************************************************************************* //
